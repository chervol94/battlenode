var socket = io.connect("192.168.12.129:8080",{"forceNew":true});//Abrimos una conexion con el servidor
...
window.onload = function(){
  ...
  createBoard2(1350,0,"transparent");//Creamos otro tablero
  createPilot2(1325,25,"red");
  //alert(socket.url);
  ...
  socket.on("gameintr",function(ret){//Cuando recibimos la respuesta
    ...
    window.location.replace("http://192.168.12.129:8080");//Lo rederigimos a la raiz de la web
  });
  ...
  socket.on("checkAvailableRet",function(reta){//Cuando el servidor emite la respuesta a checkAvailable ejecutamos esta funcion
    if(reta == 0){// Si la respesta del servidor es negativa
      ...
      window.location.replace("http://192.168.12.129:8080");  //Redirigimos a la raiz.
    }else{
  ...
  socket.on("EndGameWin",function(){//Indicamos al cliente que gana
    ...
    window.location.replace("http://192.168.12.129:8080/rank")//redirigimos al ranking
  });
  socket.on("EndGameLose",function(){//Indicamos al cliente que pierde
    alert("Has perdido, desgraciadamente no entras en el Ranking");
    window.location.replace("http://192.168.12.129:8080");//redirigimos a raiz
  });
  ...
  function createPilot2(x,y,color){//Lo mismo que la anterior pero con las cordenadas adecuadas para colocarlo en el tablero del enemigo
    ...
    board.text(xt,yt,"Listo");
    pilotEn = board.circle(x,y,8).attr({
      fill:color,
      stroke:"black",
      strokewidth:"1px",
    });
